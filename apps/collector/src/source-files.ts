...
export async function discoverPromptFiles(
  ...
  if (originalStat.isFile()) {
    if (extname(safeRoot).toLowerCase() !== ".jsonl") {
      throw new Error(`${sourceLabel} source must be a .jsonl file or directory`);
    }
    return [safeRoot];
  }
  if (!originalStat.isDirectory()) {
    throw new Error(`${sourceLabel} source must be a file or directory`);
  }

  const files: string[] = [];
  async function walk(directory: string, depth: number): Promise<void> {
    if (depth > MAX_DEPTH) {
      throw new Error(`${sourceLabel} source exceeds the directory depth limit`);
    }
    ...
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      if (entry.isDirectory()) {
        ...
        if (files.length > MAX_FILES) {
          throw new Error(`${sourceLabel} source exceeds the file count limit`);
        }
      }
    }
  }
...
export function discoverCodexFiles(sourcePath: string): Promise<string[]> {
  return discoverPromptFiles(sourcePath, "Codex");
}
