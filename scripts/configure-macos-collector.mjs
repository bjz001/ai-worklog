...
export function buildCollectorEnvironment(options) {
  ...
  const lines = [
    ...
    `ZCODE_SOURCE_INSTANCE_ID=${existingOr(options.existing, "ZCODE_SOURCE_INSTANCE_ID", "macos-zcode")}`,
    `ZCODE_HOOK_SPOOL=${existingOr(options.existing, "ZCODE_HOOK_SPOOL", join(dataDirectory, "zcode-spool"))}`,
    `ZCODE_CONFIG_PATH=${existingOr(options.existing, "ZCODE_CONFIG_PATH", join(options.home, ".zcode", "cli", "config.json"))}`,
    `DSH_SOURCE_INSTANCE_ID=${existingOr(options.existing, "DSH_SOURCE_INSTANCE_ID", "macos-dsh")}`,
    `DSH_SOURCE_PATH=${existingOr(options.existing, "DSH_SOURCE_PATH", existingOr(options.existing, "DSH_HOME", join(options.home, ".dsh")))}`,
    ...
    `COLLECTOR_BLOB_ROOT=${existingOr(options.existing, "COLLECTOR_BLOB_ROOT", join(dataDirectory, "blobs"))}`,
